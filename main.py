...
#Was drawing the landmarks on the input image previously, removed for beautiness and clarity sake
def DrawOnImage(img):
...
while True:
    if programExecuting == True:
        window = gw.getWindowsWithTitle("Zoom")[0]
        #window.minimize()
        window.restore()
    ...
    if keyboard.is_pressed("x"):
        programExecuting = not programExecuting